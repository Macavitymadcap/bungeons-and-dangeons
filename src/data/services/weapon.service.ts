...
export class WeaponService {
  ...
  public getWeaponsByType(): Record<string, Weapon[]> {
    ...
    const groupedByType = allWeapons.reduce(
      (acc, weapon) => {
  ...
  public getMeleeWeapons(): Weapon[] {
    return this.getAllWeapons().filter(weapon => weapon.type.includes('Melee'));
  }
  ...
  public getWeaponsByProperty(propertyName: string): Weapon[] {
    return this.getAllWeapons().filter(weapon =>
      weapon.properties.includes(propertyName as any),
    );
  }
